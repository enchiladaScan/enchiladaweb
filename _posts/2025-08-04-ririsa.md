---
layout: caps title: "2.5 Jigen no Ririsa" 
date: 2025-09-05 series: "2.5-jigen-no-ririsa" 
...
<ul> {% assign caps = site['2.5-jigen-no-ririsa'] | sort: 'capitulo' %} {% if caps %} {% for c in caps %} <li><a href="{{ site.baseurl }}{{ c.url }}">{{ c.title }}</a></li> {% endfor %} {% else %} <li class="no-links">Aún no hay capítulos de este manga.</li> {% endif %} </ul>